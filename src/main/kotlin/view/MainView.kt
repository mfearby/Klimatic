...
class MainView: View("Klimatic") {
    ...
    private var forecastView: TableView<Forecast> by singleAssign()
    ...
    override val root = borderpane {
        ...
        center = vbox {
            ...
            vbox {
                ...
                divider = hbox()
                sevenDayLabel = label()
                forecastView = tableview {
                    readonlyColumn("Date", Forecast::date) {
                        minWidth = 150.0
                    }
                    readonlyColumn("Min", Forecast::minTemp)
                    readonlyColumn("Max", Forecast::maxTemp)
                    readonlyColumn("Humidity", Forecast::humidity)
                }
            }
        }
    }
    ...
    private fun updateUI(city: String) {
        val forecasts = forecastController.forecasts
        val today = forecasts.first()
        cityName.text = "Forecast for $city on ${today.date}"
        ...
        sevenDayLabel.text = "Two Week Weather Forecast"
        forecastView.items = forecasts
    }
